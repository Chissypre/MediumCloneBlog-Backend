...
server.post('/upload', upload.single('file'), async (req, res) => {
  try {
    ...
    const fileStr = req.body.data.buffer.toString('base64'); 
    const uploadResponse = await uploadToCloudinary(fileStr);
    console.log(uploadResponse);
    res.json(uploadResponse);
  } catch (err) {
...
server.post("/google-auth", async (req, res) => {
  ...
  getAuth()
    ...
    .then(async (decodedUser) => {
      ...
      if (user) {
        if (!user.google_auth) {
          return res.status(403).json({ "error": "This email was signed up without goggle> Please log in with password to access the account" })
        }
      }
      else {
        let username = await generateUsername(email)
    ...
    .catch(err => {
      return res.status(500).json({ "error": "Failed to authenticate you with goggle . Try with some other goggle account" })
    })
})
...
.then((user) =>{
  if(user.google_auth){
    return res.status(403).json({"error":"You can't change account password because you logged in through goggle"})
  }
...
server.post("/search-blogs", (req, res) => {
  let {tag, query, page, limit, author, eliminate_blog} = req.body;
  let findQuery;
  if(tag){
...
server.post('/get-profile', (req, res) => {
  ...
  .select("-personal_info.password -google-auth -updatedAt -blogs")
  .then(user =>{
...
server.post('/create-blog', verifyJWT, async (req, res) => {
  ...
  if(isAdmin){
    ...
    if (!draft) {
     ...
     let bannerData;
    if (!draft && banner) {
      bannerData = await uploadToCloudinary(banner);
    } 
  ...
  Blog.findOneAndUpdate({blog_id},{title, des, content, tags, blog_id, draft: draft ? draft : false,
    banner})
    .then(() =>{
  ...
  blog.save().then(blog => {
    ...
    return res.status(500).json({error:"you dont have permissions to create any blog"})
  }
...
const deleteComments = (_id) => {
  ...
  .then(comment =>{
    ...
    Blog.findOneAndUpdate({_id:comment.blog_id}, {$pull: {comments:_id}, $inc:{"activity.total_comments":-1}, 
...
server.post ("/delete-blog", verifyJWT, (req, res) => {
  ...
  if(isAdmin){
    ...
    return res.status(500).json({error: "You dont have permission to delete the blog"})
  }
